...
public class CambioController {
	...
	private final CotacaoBCBClient cotacaoBCBClient;
	private final CacheManager cacheManager;
	
	private String CAMBIO_CACHE = "cambioCache";

	public CambioController(CambioRepository cambioRepository, CotacaoBCBClient cotacaoBCBClient, CacheManager cacheManager) {
		super();
		this.cambioRepository = cambioRepository;
		this.cotacaoBCBClient = cotacaoBCBClient;
	...
	public ResponseEntity<CambioEntity> getCambio(
			@PathVariable double valor, @PathVariable String origem, @PathVariable String destino) throws Exception {
	...
	public CambioEntity getCambioFromBancoCentral(String origem, String destino) {
		...
		CotacaoResponse cotacaoOrigem = cotacaoBCBClient.getCotacaoMoedaDia(origem, "10-16-2024");
		double fatorOrigem = cotacaoOrigem.getValue().get(0).getCotacaoVenda();
		if (destino.equals("BRL")) {
			fator = fatorOrigem;
		} else {
			CotacaoResponse cotacaoDestino = cotacaoBCBClient.getCotacaoMoedaDia(destino, "10-16-2024");
